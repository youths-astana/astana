(function () {
  ...
  function clubView(id) {
    ...
    return '<article class="screen tight">' +
      ...
      '<div class="contacts">' +
      (tel ? '<a class="wa" href="https://wa.me/' + tel + '" target="_blank" rel="noopener">WhatsApp<small>' + esc(T.write) + '</small></a>' : '') +
      ((c.ig || c.tg) ? '<div class="grid2">' +
        (c.ig ? '<a class="social" href="https://instagram.com/' + esc(encodeURIComponent(c.ig)) + '" target="_blank" rel="noopener">Instagram</a>' : '') +
        (c.tg ? '<a class="social" href="https://t.me/' + esc(encodeURIComponent(c.tg)) + '" target="_blank" rel="noopener">Telegram</a>' : '') +
        '</div>' : '') +
      (tel ? '<a class="tel" href="tel:+' + tel + '">' + esc(c.tel) + '</a>' : '') +
      '</div>' +
  ...
  function aboutView() {
    ...
    return '<article class="screen about">' + LOGO(58) +
      ...
      T.about_s.map(function (s) { return '<section><h2>' + esc(s.h) + '</h2><p>' + esc(s.p) + '</p></section>'; }).join('') +
